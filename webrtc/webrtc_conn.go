...
type webRTCConn struct {
	readBufferCount int
	remoteRtspAddr  string // retrived from zk node the format of key-value looks like (/{device-id}/{topic}:podIP)
	pathName        string
...
func (c *webRTCConn) log(level Level, format string, args ...interface{}) {
...
func (c *webRTCConn) runInner(ctx context.Context) error {
	errRtspStream := make(chan error)
	streamReady := make(chan struct{})

	go func() {
		errRtspStream <- c.readRtspStream(ctx, streamReady)
	}()

	_, ok := <-streamReady
	if !ok {
		return <-errRtspStream
	}

	tracks, err := c.allocateTracks(c.stream.medias())
	if err != nil {
		return err
	}
	...
	pc.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		c.log(Debug, "peer connection state: "+state.String())
...
outer:
	for {
		...
		case candidate := <-localCandidate:
			c.log(Debug, "local candidate: %+v", candidate.Candidate)
		...
		case candidate := <-remoteCandidate:
			c.log(Debug, "remote candidate: %+v", candidate.Candidate)
	...
	c.log(Info, "peer connection established, local candidate: %v, remote candidate: %v",
	...
	for _, track := range tracks {
		ctrack := track
		c.stream.readerAdd(c, track.media, track.format, func(dat formatprocessor.Data) {
			ringBuffer.Push(func() {
				ctrack.cb(dat, ctx, writeError)
			})
		})
	}
	defer c.stream.readerRemove(c)

	c.log(Info, "is reading from remote '%s', %s",
		buildRtspPath(c.remoteRtspAddr, c.pathName), sourceMediaInfo(gatherMedias(tracks)))
