...
func NewLogger(level Level, destinations map[Destination]struct{}, fpath string) (*Logger, error) {
	lh := &Logger{
		level:        level,
	...
	if _, ok := destinations[DestinationFile]; ok {
		var err error
		lh.file, err = os.OpenFile(filepath.Clean(fpath), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0600)
		if err != nil {
			lh.Close()
			return nil, err
		}
...
func (lh *Logger) Log(level Level, format string, args ...interface{}) {
	...
	if _, ok := lh.destinations[DestinationFile]; ok {
		...
		_, _ = lh.file.Write(lh.fileBuffer.Bytes())
	}
	...
	if _, ok := lh.destinations[DestinationSyslog]; ok {
		...
		_, _ = lh.syslog.Write(lh.syslogBuffer.Bytes())
	}
}
